...
import com.mmasesoriasTest.dao.UsersDAO;
...
public class MMAsesoriasTestApplicationTests {
	...
	public void givenListOfUsers_whenGetAllUsers_thenReturnUsersList() throws Exception{
		...
		listUsers.add(User.builder().id(1L).firstname("This is user 1").build());
		listUsers.add(User.builder().id(2L).firstname("This is user 2").build());
	...
	public void givenUserId_whenGetUserById_thenReturnUserObject() throws Exception{
		...
		response.andExpect(status().isOk())
				...
				.andExpect(jsonPath("$.description", is(user.getFirstname())));
	}
